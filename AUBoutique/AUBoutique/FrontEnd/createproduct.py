...
class MainWindow(QMainWindow):
    ...
    def initUI(self, username):
        ...
        Submit.clicked.connect(lambda: self.validate_inputs(name.text(), price.text(), desc.text(), quantity.text(), username))
        
    def validate_inputs(self, name, price, description, quantity, username):
        ...
        if quantity_value <1:
            ...
            return
        currency = client.getUserCurrency(username)
        client.add_product(name, quantity_value, price_value, description, self.imageFile, currency)
